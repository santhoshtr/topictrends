...
// Hover text with the English label, when it differs from the local title.
function setEnglishHover(el, title, titleEn) {
	if (titleEn && titleEn !== title) {
		el.title = formatTitle(titleEn);
	}
}
...
function createCategoryPill(category, wiki, trendPath, startDate, endDate) {
	...
	setEnglishHover(labelLink, category.title, category.title_en);
...
function createArticleCell(article, wiki, trendPath, startDate, endDate) {
	...
	setEnglishHover(titleLink, article.title, article.title_en);
