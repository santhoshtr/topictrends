...
function renderArticles(container, articles, wiki) {
	...
	// Use category QID from the parent <details> as a unique scope for radio names/ids
	...
	// Hide tabs with no data; if active tab becomes empty, fall back to Top
	if (gains.length === 0) {
		radioUp.hidden = true;
		lblUp.hidden = true;
	}
	...
	// JS-driven panel switching (IDs are dynamic so pure-CSS :checked ~ panel won't work here)
...
function createArticleElement(article, wiki) {
	...
	const formatDate = (date) => date.toISOString().split("T")[0];
	plotLink.href = `/pageviews/trends?type=article&wiki=${wiki}&start_date=${formatDate(startDate)}&end_date=${formatDate(endDate)}&article=${article.article_title}`;
