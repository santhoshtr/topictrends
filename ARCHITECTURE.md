...
At load time the engine translates each QID to the current dense article ID via `art_original_to_dense` and builds a `DailyPageViewData` — two parallel sorted `Vec<u32>` arrays of dense IDs and view counts, holding only articles that actually had views on that date. Lookup is `O(log n)` via binary search; iteration yields `(dense_id, views)` pairs in dense-id order. For enwiki this drops per-day footprint from ~28 MB (dense `Vec<u32>` of length 7 M, mostly zeros) to ~6-12 MB and makes the scatter step in `get_top_categories` / `get_top_articles` skip zeros entirely — ~1.5 M non-zero iterations per day rather than 7 M.

The per-date cache is bounded (FIFO eviction by insertion order) to keep RSS under control over time. The cap is configured via `TOPICTREND_PAGEVIEW_CACHE_DAYS` (default `120`, `0` disables the bound). To make eviction race-free under concurrent requests, `load_history_for_date_range` returns an `Arc`-snapshot of the requested range; the aggregation paths iterate that snapshot instead of re-acquiring the cache lock, so an entry evicted by another request mid-query is still alive for the caller that needed it.
